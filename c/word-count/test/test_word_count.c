...
{
   // All words counted?
   TEST_ASSERT_EQUAL_INT_MESSAGE(expected_word_count, actual_word_count, "All words not counted");

   // now test the word count for the words...
   for (int index = 0; index < MAX_WORDS; index++) {
      TEST_ASSERT_EQUAL_INT_MESSAGE(expected_solution[index].count,
                        actual_solution[index].count, "The word count");
      TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE(expected_solution[index].text,
                                    actual_solution[index].text, STRING_SIZE, "data not equal");
   }
}
